    ...
    public class LoggerSubTree : SubTree
    {
        ...
        protected override Node BuildTree()
        {
            var repeat = new RepeatNode(new SelectorNode()
                ...
                // Find a collect sticks
                .AddChild(new SequencerNode()
                    .AddChild(new Action_FindNearestWorldObject<Sticks>("targetObject"))
                    .AddChild(new SequencerNode()
                        .AddChild(new InverterNode(new CheckNullNode("targetObject")))
                        .AddChild(new Action_MoveTo("targetObject"))
                    )
                    .AddChild(new Action_CollectSticks("targetObject"))
                )
                // Find and chop a tree
                .AddChild(new SequencerNode()
                    .AddChild(new Action_FindNearestWorldObject<Tree>("targetObject"))
                    .AddChild(new SequencerNode()
                        .AddChild(new InverterNode(new CheckNullNode("targetObject")))
                        .AddChild(new Action_MoveTo("targetObject"))
                    )
